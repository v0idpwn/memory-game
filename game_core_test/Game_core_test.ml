...
let card_duplication_on_board _ctx =
  ...
  let () = assert_bool "Unique card count not 2" (Array.count board.cards ~f:(fun c -> c.content = 1) = 2) in
  ()

(* If all cards are open, board is won *)
let win_condition _ctx =
  let board = Board.new_board 10 in
  let () = Array.map_inplace board.cards ~f:(fun card ->
    {card with status = Open}
  ) in
...
let open_card_win_flow _ctx =
  let board = Board.new_board 10 in
  let _ = Array.mapi board.cards ~f:(fun idx card ->
    let opened_card = {card with status = Open} in
    if idx < 19 then Array.set board.cards idx opened_card else ()) in
...
let clear_board_keeps_pairs _ctx =
  ...
  let match_card = Array.findi board.cards ~f:(fun _idx card ->
  ...
  let board = Board.clear_board board in
  let () = assert_equal ~cmp:phys_equal (Array.get board.cards 19).status Open in
  let () = assert_equal ~cmp:phys_equal (Array.get board.cards match_card_idx).status Open in
  ()

(* Running *)
let tests = "core game test" >::: [
