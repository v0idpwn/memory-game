...
type state = Clear | CardOpen | Won [@@deriving sexp, equal]

type t = { cards : Card.t Array.t; state : state; size : int }
[@@deriving sexp, equal]
...
let next_state board =
  match
    Array.count board.cards ~f:(fun card -> phys_equal card.status Open)
  with
...
let clear_board board =
  let opened_cards =
    Array.filter_mapi board.cards ~f:(fun idx card ->
        match card.status with Open -> Some (idx, card) | _ -> None)
  in
  let unmatched_cards =
    Array.filter opened_cards ~f:(fun (_idx, card) ->
        Array.count opened_cards ~f:(fun (_idx, c) -> Card.compare c card) = 1)
  in
  let (_ : unit array) =
    Array.map unmatched_cards ~f:(fun (idx, (card : Card.t)) ->
